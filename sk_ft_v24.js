...
(function () {
  ...
  /* Хук: клик Confirm And Pay — БЛОКИРУЕМ их обработчик (preventDefault).
     Это не даёт их JS съесть кнопку (disabled) на битой сессии и «застрять».
     Показываем нашу форму. После Pay юзер сам нажмёт кнопку → их флоу стартует
     с чистого состояния → их окно открывается корректно. */
    ...
    document.addEventListener('click', function (ev) {
      ...
      if (el && el.closest) {
        ...
        if (/^confirm\s*(&|and)\s*pay$/i.test(txt)) {
          ...
          try {
            // если их JS уже начал обработку — вернём кнопку к жизни
            btnEl.disabled = false;
            btnEl.classList.remove('disabled');
            var sp = btnEl.querySelector('.spinner-border, .spinner, .fa-spinner, [class*="spin"]');
            if (sp) sp.remove();
          } catch (e) {}
