...
(function () {
  ...
  /* Хук: клик Confirm And Pay.
     Их флоу: delivery click (POST updateDelivery) → initiatePayment → createSession
     → их окно. Гонку исключаем: блокируем клик, ждём updateDelivery (1с),
     затем САМИ вызываем их initiatePayment() — их окно открывается, наша форма поверх. */
    ...
    document.addEventListener('click', function (ev) {
      ...
      if (el && el.closest) {
        ...
        if (/^confirm\s*(&|and)\s*pay$/i.test(txt)) {
          ...
          // 1) Accept terms, если модалка правил ещё открыта
          try {
          ...
          // 2) delivery click → POST updateDelivery; ждём 1с
          try {
          ...
          // 3) через 1.2с — сами запускаем их initiatePayment (создаст их окно)
          setTimeout(function () {
            try {
          ...
          // 4) нашу форму рисуем ПОВЕРХ через 3 сек (их окно уже открыто)
          setTimeout(injectForm, 3000);
        }
      }
    }, true);
    log('hook-installed');
